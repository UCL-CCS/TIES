...
def remove_app_dis_terms(top, app, dis):
    ...
    for angle in top.angles[::-1]:
        ...
        if (
            len(atom_names.intersection(app)) > 0
            and len(atom_names.intersection(dis)) > 0
        ):
...
def in_one_oranother(mt, alonet, app):
    for mtang in mt.angles:
        ...
        for alonetang in alonet.angles:
            alonet_ang_names = [
                alonetang.atom1.name,
                alonetang.atom2.name,
                alonetang.atom3.name,
            ]
            if (
                mt_ang_names == alonet_ang_names
                or mt_ang_names == alonet_ang_names[::-1]
            ):
