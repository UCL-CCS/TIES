...
def find_waters(
    ...
    lig_sel="resname ged",
    wat_sel="resname WAT",
    output_file="closest_water_ids.dat",
    output_summary_file="cumulative_water_time.dat",
    ...
    for ts in u.trajectory[start_frame:number_of_frames:every_nth_frame]:
        ...
        wat_prot_res_min = np.min(
            wat_prot_shortest.reshape([len(water_o.residues), 3]), axis=1
        )
        wat_lig_res_min = np.min(
            wat_lig_shortest.reshape([len(water_o.residues), 3]), axis=1
        )
        ...
        # prepare for sorting
        chained = [
            (wid, dst) for wid, dst in zip(water_o.residues.resids, wat_others_sum)
        ]
