...
def combine(transformations):
    ...
    for transformation, replica_sets in itertools.groupby(
        transformations, key=lambda t: str(t).split("/")[-1].rsplit("_", maxsplit=1)[0]
    ):
...
dgs, sds = combine(
    transformations
)  # , root_work, filename='thrombin_lig', yrange=0.5, plots=[2, 3])
ligs["thrombin"] = sds
# complex
transformations = list(root_work.glob("thrombin/analysis/complex_l*_l*_*.json"))
dgs, sds = combine(
    transformations
)  # , root_work, filename='thrombin_complex', yrange=2.5, plots=[2,3])
...
dgs, sds = combine(
    transformations
)  # , root_work, filename='cdk2_lig', yrange=0.5, plots=[1, 3])
ligs["cdk2"] = sds
# complex
transformations = list(root_work.glob("cdk2/analysis/complex_l*_l*_*.json"))
dgs, sds = combine(
    transformations
)  # , root_work, filename='cdk2_complex', yrange=1, plots=[1,3])
complexs["cdk2"] = sds
