...
def extract_energies(location, choderas_cut=False, eq_steps=1000, whichReplica=None, diag=False):
    """
    ...
    @whichReplica - for testing, if set to a number, reado only that replica
    """
    ...
    for lambda_dir in lambda_dirs:
        ...
        for rep in sorted(lambda_dir.glob('rep[0-9]*')):
            if whichReplica is not None:
                if int(rep.name.split('rep')[1]) != whichReplica:
                    continue
                if diag:
                    whichReplica += 1
                    if whichReplica == 6:
...
analysis_dir = '/home/dresio/ucl/ties/ties20/normality_diagonal'
...
# use ptp1b-l10_l12.dat
# ptp1b-l11_l23.dat
# ptp1b-l19_l3.dat
ddgs = []
for whichR in range(1, 6):
    lig_all = extract_energies('lig', whichReplica=whichR, diag=False)
    laele_int, lavdw_int, ldvdw_int, ldele_int, lig_data = analyse(lig_all, 'lig', calc_aga_err=False, verbose=True, plot=False)
    lig_delta = laele_int + lavdw_int - ldvdw_int - ldele_int
    complex_all = extract_energies('complex', whichReplica=whichR, diag=False)
    caele_int, cavdw_int, cdvdw_int, cdele_int, complex_data = analyse(complex_all, 'complex', calc_aga_err=False, verbose=True, plot=False)
    complex_delta = caele_int + cavdw_int - cdvdw_int - cdele_int
