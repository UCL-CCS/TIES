...
def analyse(
    data, system_conf, calc_aga_err=False, sample_reps=0, verbose=False, plot=False
):
    """
...
for transformation, replica_sets in itertools.groupby(
    ...
    for rep_no in [1, 3, 5, 10, 15, 20]:
        ...
        for i in range(bs_num):  # fixme
            aele_int, avdw_int, dvdw_int, dele_int = analyse(
                all_lig,
                "All Lig",
                sample_reps=rep_no,
                calc_aga_err=False,
            ...
            bootstrapped_ddGs[rep_no].append(delta2 - delta1)
