#!/usr/bin/env python3
"""
Another variation. Calculate how much the ddG changes if you add more replicas.
...
for transformation, replica_sets in itertools.groupby(transformations, key=lambda t:str(t).split('/')[1]):
    ...
    for rep_no in range(1, 20):
        ...
        for i in range(bs_num):  # fixme
            ...
            delta1 = aele_int + avdw_int - dvdw_int - dele_int
            aele_int, avdw_int, dvdw_int, dele_int, complex_stats = analyse(all_complex, 'Complex', sample_reps=rep_no,
                                                                calc_aga_err=False, verbose=False, plot=False)
            delta2 = aele_int + avdw_int - dvdw_int - dele_int
            ddg_base = delta2 - delta1
            ...
            delta2 = aele_int + avdw_int - dvdw_int - dele_int
            ddg_plus_1 = delta2 - delta1

            print('OVERALL CHANGE: ', ddg_base - ddg_plus_1)
            # record the change in ddG obtained by adding one more replica
            bootstrapped_ddGs[rep_no].append(ddg_base - ddg_plus_1)

    # give files their own specific name
    tseed = str(time.time()).split('.')[1]

    # save bs to a file
    with open(f'analysis/direct_ddg_{transformation}_{tseed}.json', 'w') as bs_file:
