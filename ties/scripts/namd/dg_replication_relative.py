...
def analyse(
    ...
    if stats is None:
        ...
        for interaction_type, dataset in data.items():
            if interaction_type in ["avdw", "dvdw", "aele", "dele"]:
                stats[interaction_type] = get_replicas_stats(
                    dataset, sample_reps=sample_reps, stats=stats[interaction_type]
                )
...
for transformation, replica_sets in itertools.groupby(
    transformations, key=lambda t: str(t).split("/")[1]
):
    ...
    for repset in replica_sets:
        ...
        with open(repset / "analysis/lig.pkl", "rb") as FLIG, open(
            repset / "analysis/complex.pkl", "rb"
        ) as FCOMPLEX:
    ...
    for force_type in force_types:
        print(
            f"For ligand {force_type} there is together {len(all_lig[force_type][0])} replicas"
        )
    ...
    for force_type in force_types:
        print(
            f"For complex {force_type} there is together {len(all_complex[force_type][0])} replicas"
        )
    ...
    for rep_no in range(1, 20):
        ...
        for i in range(bs_num):  # fixme
            ...
            aele_int, avdw_int, dvdw_int, dele_int, lig_stats = analyse(
                all_lig,
                "All Lig",
                sample_reps=rep_no,
                calc_aga_err=False,
            ...
            aele_int, avdw_int, dvdw_int, dele_int, complex_stats = analyse(
                all_complex,
                "Complex",
                sample_reps=rep_no,
                calc_aga_err=False,
            ...
            aele_int, avdw_int, dvdw_int, dele_int, _ = analyse(
                all_lig,
                "All Lig",
                sample_reps=rep_no,
                calc_aga_err=False,
    ...
    # save bs to a file
    with open(
        f"analysis/relative_lig_dg_{transformation}_{tseed}.json", "w"
    ) as bs_file:
        bs_file.write(json.dumps(lig_dGs))
    with open(
        f"analysis/relative_complex_dg_{transformation}_{tseed}.json", "w"
    ) as bs_file:
