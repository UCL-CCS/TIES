#!/usr/bin/env python3
"""
Another variation. Calculate how much the dG changes if you add more replicas.
E.g. if you take 1 replicas, and the energy is 2 kcal/mol, but with 5 replicas, it is 2.3,
the report +0.3 with 5 replicas. Create a distribution of that.
However, for now it does this:
Select 1 and check +1, select 2 replicas, add 1, select 3 replicas, add 1,
In other words, evaluate the addition of 1 replica at eacth stage to see how much it changes the ddG

Arg1: sample size, how many times to carry out (bootstrapping size)
Arg2: which case ie l1_l4
...
def get_replicas_stats(dataset, sample_reps=None, stats=None):
    """
    To do: obtains stats for each replica separately, as well as for all of them together merged,
    sample_reps: how many replicas from which to sample. None means merge all replicas. An integer
        here will pick a random set. A list will choose specific replicas.
    todo
    """
    meta = {
        # combined
    ...
    for lambda_val, replicas in dataset.items():
        ...
        if stats is None:
            merged_replicas = []
            if sample_reps is None:
                for rep in replicas:
                    merged_replicas.extend(rep)
            elif isinstance(sample_reps, int) and sample_reps > 0:
                for rep in random.choices(replicas, k=sample_reps):
                    merged_replicas.extend(rep)
        else:
            # we work on previously computed data,
            # just add one more replica to it, so set merged to what it was previously
            merged_replicas = stats['merged_replicas'][lambda_val]
            # add one more replica picked by random
            merged_replicas.extend(random.choice(replicas))
        ...
        # remember the merged replicas used
        meta['merged_replicas'][lambda_val] = merged_replicas
...
def analyse(data, system_conf, calc_aga_err=False, sample_reps=0, verbose=False, plot=False, stats=None):
    """
    ...
    if stats is None:
        # get all the replicsa that you need
        stats = {}
        ...
        # add just one more replica to the existing replicas
        for interaction_type, dataset in data.items():
            if interaction_type in ['avdw', 'dvdw', 'aele', 'dele']:
                stats[interaction_type] = get_replicas_stats(dataset, sample_reps=sample_reps, stats=stats[interaction_type])
...
---------------------------------------------------------"""
    ...
    return aele_int, avdw_int, dvdw_int, dele_int, stats
...
for transformation, replica_sets in itertools.groupby(transformations, key=lambda t:str(t).split('/')[1]):
    ...
    lig_dGs = {}
    complex_dGs = {}
    for rep_no in range(1, 20):
        lig_dGs[rep_no] = []
        complex_dGs[rep_no] = []
        for i in range(bs_num):  # fixme
            # carry out this analysis on X replicas
            aele_int, avdw_int, dvdw_int, dele_int, lig_stats = analyse(all_lig, 'All Lig', sample_reps=rep_no,
                                                                calc_aga_err=False, verbose=False, plot=False)
            lig_dG1 = aele_int + avdw_int - dvdw_int - dele_int

            aele_int, avdw_int, dvdw_int, dele_int, complex_stats = analyse(all_complex, 'Complex', sample_reps=rep_no,
                                                                calc_aga_err=False, verbose=False, plot=False)
            complex_dG1 = aele_int + avdw_int - dvdw_int - dele_int

            # add 1 more arbitrary replica and carry out the same analysis
            aele_int, avdw_int, dvdw_int, dele_int, _ = analyse(all_lig, 'All Lig', sample_reps=rep_no,
                                                                        calc_aga_err=False, verbose=False, plot=False,
                                                                        stats=lig_stats)
            lig_dG2 = aele_int + avdw_int - dvdw_int - dele_int
            aele_int, avdw_int, dvdw_int, dele_int, _ = analyse(all_complex, 'Complex', sample_reps=rep_no,
                                                                            calc_aga_err=False, verbose=False,
                                                                            plot=False, stats=complex_stats)
            complex_dG2 = aele_int + avdw_int - dvdw_int - dele_int

            # what is the change when we add one more replica?
            lig_change = lig_dG1 - lig_dG2
            complex_change = complex_dG1 - complex_dG2

            # record the change in ddG obtained by adding one more replica
            lig_dGs[rep_no].append(lig_change)
            complex_dGs[rep_no].append(complex_change)

    # give files their own specific name
    tseed = str(time.time()).split('.')[1]

    # save bs to a file
    with open(f'analysis/relative_lig_dg_{transformation}_{tseed}.json', 'w') as bs_file:
        bs_file.write(json.dumps(lig_dGs))
    with open(f'analysis/relative_complex_dg_{transformation}_{tseed}.json', 'w') as bs_file:
        bs_file.write(json.dumps(complex_dGs))
