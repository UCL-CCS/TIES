...
for case in cases:
    ...
    for i in range(1, 4 + 1):
        lig = extract_energies(
            Path("/home/dresio/ucl/validation/replica20/")
            / protein
            / f"set{i}"
            / case
            / "lig"
    ...
    laele_int, lavdw_int, ldvdw_int, ldele_int, lig_data = analyse(
        lig_all, "lig", calc_aga_err=calc_aga_err, verbose=True
    )
    lig_delta = laele_int + lavdw_int - ldvdw_int - ldele_int

    complexes = []
    for i in range(1, 4 + 1):
        complex = extract_energies(
            Path("/home/dresio/ucl/validation/replica20/")
            / protein
            / f"set{i}"
            / case
    ...
    caele_int, cavdw_int, cdvdw_int, cdele_int, complex_data = analyse(
        complex_all, "complex", calc_aga_err=calc_aga_err, verbose=True
    )
