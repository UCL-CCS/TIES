...
##BSUB -q scafellpikeKNL # can be used a little, 12x4, try 65x1
##BSUB -q universeScafellPike - cannot be used
##BSUB -q scafellpikeI - cannot be used
#BSUB -W 35:00
#BSUB -x

SYSTEM=$HCBASE/resp_ff/tyk2_l1_l3
NP=64
...
# FIXME - MANUAL ------------------------------------------------
declare -a lambdas=(0.05	0.00	0.20	1.00	0.95	0.95	0.95)
declare -a reps=(1		2		3		1		5		4		3)
array_id=$(( $array_id - 1 )) # start indexes from 0
lambda=${lambdas[$array_id]}
replica=${reps[$array_id]}
echo "manual lambda" $lambda
echo "manual rep" $rep
# FIXME - MANUAL END ------------------------------------------------
