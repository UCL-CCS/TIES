#!/usr/bin/env python3
"""
This is another variant. Shunzhou requested ddG calculated using 1 replica,
with the replicas matched by number in each lambda window.

So this script, for 5 replicas, for each lambda it takes replica 1
and calculated ddG, then replica 2 and calculates ddG etc,
giving 5 ddGs from a set of 5 replicas.
"""
...
def extract_energies(
    ...
    for lambda_dir in lambda_dirs:
        ...
        for rep in sorted(lambda_dir.glob("rep[0-9]*")):
            ...
            # check if the replica should be included
            if pick_replicas is not None:
                rep_num = int(str(rep).split("rep")[1])
                if rep_num not in pick_replicas:
                    continue
...
for rep_in in range(1, 5 + 1):
    print(f"Replica {rep_in}")
    choderas_cut = False
    calc_aga_err = False
    lig_all = extract_energies("lig", choderas_cut=choderas_cut, pick_replicas=[rep_in])
    laele_int, lavdw_int, ldvdw_int, ldele_int, lig_data = analyse(
        lig_all, "lig", calc_aga_err=calc_aga_err, verbose=False, plot=False
    )
    lig_delta = laele_int + lavdw_int - ldvdw_int - ldele_int
    complex_all = extract_energies(
        "complex", choderas_cut=choderas_cut, pick_replicas=[rep_in]
    )
    caele_int, cavdw_int, cdvdw_int, cdele_int, complex_data = analyse(
        complex_all, "complex", calc_aga_err=calc_aga_err, verbose=False, plot=False
    )
    complex_delta = caele_int + cavdw_int - cdvdw_int - cdele_int
