...
def command_line_script():
    ...
    parser.add_argument(
        ...
        "--q-net-tolerance",
        metavar="decimal",
        dest="net_charge_threshold",
        type=float,
        required=False,
        default=0.1,
        help="The maximum difference in charge between two ligands. Default 0.1e",
    )
    parser.add_argument(
        "-use-input-q",
        "--use-ligand-charges",
        metavar="boolean",
        dest="ligands_contain_q",
    ...
    parser.add_argument(
        ...
        "--align-ligands-mcs",
        metavar="boolean",
        dest="align_molecules_using_mcs",
        type=ArgparseChecker.str2bool,
        required=False,
        default=True,
    ...
    parser.add_argument(
        ...
        metavar="str or bool",
        dest="logging_level",
        type=ArgparseChecker.logging_lvl,
        required=False,
        default="INFO",
    ...
    parser.add_argument(
        ...
        "--ambertools-home",
        metavar="path",
        dest="ambertools_home",
        type=ArgparseChecker.ambertools_home,
        required=False,
    ...
    parser.add_argument(
        "-match",
        "--manual-match",
        metavar="file or pair",
        dest="manually_matched_atom_pairs",
        type=pathlib.Path,
        required=False,
    ...
    parser.add_argument(
        "-seeds",
        "--superimposition-starting-pairs",
        metavar="pairs",
    ...
    parser.add_argument(
        ...
        "--manual-unmatch",
        metavar="file or pair",
        dest="manually_mismatched_pairs",
        type=pathlib.Path,
        required=False,  # fixme - implement
        help="A path to a file that contains atom-pairs (one per line)."
        'Each pair should be separated by dash "-" character.'
    ...
    parser.add_argument(
        "-disjoint-allowed",
        "--disjoint-appearing-disappearing",
        metavar="boolean",
    ...
    parser.add_argument(
        ...
        required=False,
        default="namd2.14",
        help='Generate input files for the selected engine. Default is "namd2.14". ',
    )
    parser.add_argument(
        "-dirtree",
        "--engine-tree",
        metavar="bool",
        dest="lambda_rep_dir_tree",
        type=ArgparseChecker.str2bool,
        required=False,
        default=False,
    ...
    parser.add_argument(
        ...
        "--ignore-charges",
        metavar="boolean",
        dest="ignore_charges_completely",
        type=ArgparseChecker.str2bool,
        required=False,
        default=False,
    ...
    if command == "rename":
        ...
        if len(ligands) != 2:
            raise Exception(
                "ERROR: to rename atoms to be unique across two ligands, "
                "you have to provide exactly two ligands with -l."
                "E.g. ties rename -l init.mol2 final.mol2"
            )
        logger.info(
            "Atom names will be renamed to ensure that the atom names are unique across the two molecules."
        )
        ...
        if len(ligands) > 1:
            raise Exception(
                "ERROR: too many ligands ligand (-l). "
                "Use one ligand when assigning coordinates from another file."
            )
        elif config.coordinates_file is None:
            raise Exception(
                "ERROR: no file with the coordinates found. Please add a file with coordinates (-crd)."
            )
        elif args.output_filename is None:
            raise Exception(
                "ERROR: no output file provided for the mergecrd. Please use (-o), e.g. -o output.mol2 ."
            )

        # assign coordinates
        ligands[0].overwrite_coordinates_with(
            config.coordinates_file, args.output_filename
        )
        sys.exit()
    ...
    pairs = [
        ties.pair.Pair(ligA, ligZ, config)
        for ligA, ligZ in itertools.combinations(ligands, r=2)
    ]
    ...
    for pair in pairs:
        ...
        hybrid.write_mol2()
        logger.info(
            f"Compared pair of ligands: {time.perf_counter() - start_time:.1f} s"
        )
