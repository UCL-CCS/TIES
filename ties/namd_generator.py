...
def getSuptop():
    ...
    leftlig_atoms, leftlig_bonds, rightlig_atoms, rightlig_bonds, mda_l1, mda_l2 = (
        get_atoms_bonds_from_file(
            "namd_tests/l18-l39/init_l18.mol2", "namd_tests/l18-l39/final_l39.mol2"
        )
    )
    ...
    suptops = superimpose_topologies(
        ligand1_nodes.values(), ligand2_nodes.values(), starting_node_pairs=[(c11, c33)]
    )
    ...
    for atomName1, atomname2 in present:
        assert suptop.contains_atom_name_pair(atomName1, atomname2), (
            f"({atomName1}, {atomname2}) not found"
        )
    ...
    for atom in mda_l2.atoms:
        if not suptop.contains_right_atom_name(atom.name):
            ...
            line = (
                ...
                f"{atom.resid:>4d}    "
                f"{atom.position[0]:>8.3f}{atom.position[1]:>8.3f}{atom.position[2]:>8.3f}"
                f"{1.0:>6.2f}{APPEARING_ATOM:>6.2f}" + (" " * 11) + "  " + "  " + "\n"
            )
            FOUT.write(line)
...
def write_merged(suptop, merged_filename):
    ...
    with open(merged_filename, "w") as FOUT:
        ...
        FOUT.write(f"{suptop.get_unique_atom_count():d} {len(bonds):d}" + os.linesep)
        # mole type
        ...
        all_atoms = [
            left for left, right in suptop.matched_pairs
        ] + suptop.get_unmatched_atoms()
        # reorder the list according to the ID
        all_atoms.sort(key=lambda atom: suptop.get_generated_atom_id(atom))

        for atom in all_atoms:
            FOUT.write(
                f"{suptop.get_generated_atom_id(atom)} {atom.name} "
        ...
        for bond_from_id, bond_to_id, bond_type in sorted(list(bonds)):
            ...
            FOUT.write(
                f"{bond_counter} {bond_from_id} {bond_to_id} {bond_type}" + os.linesep
            )
            bond_counter += 1
