...
def test_no_same_atom_names(indole_cl1):
    ...
    ties.topology_superimposer.SuperimposedTopology.rename_ligands(
        indole_cl1, dual_ring_cmp
    )
    intersection = {a.name for a in indole_cl1}.intersection(
        {a.name for a in dual_ring_cmp}
    )
