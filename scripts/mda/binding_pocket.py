...
binding_site = sys.select_atoms(f'not resname WAT and not resname ged and around 5 (resname ged)')
...
for lamdir in glob.glob(r'complex/lambda_[0-1].[0-9]*'):
	...
	for rep in glob.glob(os.path.join(lamdir, 'rep[0-9]*')):
		...
		binding_pocket = u.select_atoms(f'not resname ged and around 5 resname ged', updating=True)
